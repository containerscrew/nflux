...
fn handle_ipv4_packet(
    ...
    match ipv4hdr.proto {
        IpProto::Tcp => {
            handle_tcp_packet(ctx,  source, destination, direction, is_ether, configmap.log_every)
        },
        IpProto::Udp => {
            if configmap.enable_udp == 1 {
                handle_udp_packet(ctx, source, destination, direction, is_ether,configmap.log_every)
            } else {
                // UDP traffic monitoring is disabled
                Ok(TC_ACT_PIPE)
            }
        },
        IpProto::Icmp => handle_icmp_packet(ctx, source, destination, direction,configmap.log_every),
        _ => Ok(TC_ACT_PIPE),
    }
}
